...
profileRoute.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // ⭐ VERCEL FIX: Buffer ko Base64 Data URI mein convert karein
      ...
      // 2. Cloudinary configuration push (Ab hum direct Data URI bhej rahe hain)
      const imageRes = await cloudinaryUploader.upload(fileUrl, {
        folder: "social_media_profiles", // Cloudinary cloud directory name
        resource_type: "image"
      });
      ...
      return res.status(500).json({ 
        success: false, 
        message: error.message || "Image upload failed. Server Error." 
      });
    }
    // Note: finally block aur fs.unlinkSync bilkul hata diya hai 
    // kyunki RAM (buffer) se file code execution ke baad khud clear ho jati hai.
  }
);